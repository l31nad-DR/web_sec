...
class User(UserMixin):
    ...
    @staticmethod
    def get_by_credentials(username, hex_passwd):
        conn = sqlite3.connect('users.db')
        c = conn.cursor()


        query = f"SELECT id, username FROM userdata WHERE username = '{username}' AND password = '{hex_passwd}'"
...
def login():
    if request.method == 'POST':
        ...
        user = User.get_by_credentials(username, hex_passwd)
        if user:
            login_user(user)
